angular.module('MetronicApp').controller('themeController', function($rootScope, $scope, req,$state) {
	$scope.$on('$viewContentLoaded', function() {
		...
		$scope.saveType = function() {
			...
			if(id){
				...
				req('POST', 'theme/save', a, function(res) {
					// 请求成功执行代码
					if(res.code==0){
						alert(res.msg);
						refresh();
		...
		$scope.delType = function(data) {
			...
			req('POST', 'theme/del', a, function(res) {
				// 请求成功执行代码
				if(res.code==0){
					alert(res.msg);
					$state.go('theme',{},{reload:true});
