angular.module('MetronicApp').controller('unitController', function($rootScope, $scope, req,uploadFile,$state,$q) {
	$scope.$on('$viewContentLoaded', function() {
		...
		$scope.saveType = function() {
			...
			if(id){
				...
				console.log(a);
				req('POST', 'unit/modify', a, function(res) {
					//请求成功执行代码
					if(res.code==0){
						alert(res.msg);
						refresh();
				...
				console.log(a);
				req('POST', 'unit/save', a, function(res) {
					// 请求成功执行代码
					if(res.code==0){
						alert(res.msg);
						refresh();
		...
		$scope.delType = function(data) {
			...
			req('POST', 'unit/del', a, function(res) {
				// 请求成功执行代码
				if(res.code==0){
					alert(res.msg);
					refresh();
		...
		function init() {//初始化
			...
			req('GET', 'unit/type/all', {}, function(res) {
				// 请求成功执行代码
				if(res.code==0){
					let list = res.result;
					$scope.typeList = list;
		...
		$('#file_upload').change(function (e) {
			...
			uploadFile('unit/upload',files).then(function (res) {
